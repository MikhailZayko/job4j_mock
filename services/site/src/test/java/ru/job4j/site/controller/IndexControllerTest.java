...
class IndexControllerTest {
    ...
    void whenGetIndexPageExpectModelAttributeThenOk() throws JsonProcessingException {
        ...
        Set<ProfileDTO> profiles = Set.of(
                new ProfileDTO(1, "user1", "exp", 1, Calendar.getInstance(), Calendar.getInstance()),
                new ProfileDTO(2, "user2", "exp", 2, Calendar.getInstance(), Calendar.getInstance())
        );
        ...
        when(profilesService.getProfilesByInterviews(listInterviews)).thenReturn(profiles);
        ...
        assertThat(actualProfiles).usingRecursiveComparison().isEqualTo(profiles);
    }
}